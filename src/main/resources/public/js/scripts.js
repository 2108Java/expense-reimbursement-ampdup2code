...
if (document.URL.includes("employee-view-past-tickets.html") ) {
	window.onload = getallticket;
}else if ( document.URL.includes("finance-view-past-tickets.html") ) {
	window.onload = getallticket;
...
async function getallticket(){
		...
		const BASE_FINANCE_URL = "http://localhost:7000/VIEWPASTTICKETFILLALL";
...
function addallticketf(rembobj){
...
function addRowf(rem){
    ...
    let tableBody = document.getElementById("rembTableBodyf");
